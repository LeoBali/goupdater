...
import (
 ...
 func (p *Systray)  SetLink(alink string) {
	link = alink
 }
	 ...
	 case NotifyIconMessageId:
		...
		case WM_LBUTTONUP:
			...
			case NIN_BALLOONSHOW:
				log.Println("balloon show")
			case NIN_BALLOONUSERCLICK:
				log.Printf("opening url %s", link)
				exec.Command("rundll32", "url.dll,FileProtocolHandler", link).Start()
			case WM_MOUSEMOVE:
				log.Println("tray icon mouse move")
			case WM_LBUTTONDOWN:
				log.Println("tray icon left button down")
			default:
				log.Printf("lParam %x\r\n", lparam)
		}
